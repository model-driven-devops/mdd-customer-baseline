import csv
from nautobot import Nautobot #pip install nautobot-client
...
NETBOX_DEVICES_PATH = './docs/netbox_devices.csv'
NETBOX_SITES_PATH = './docs/netbox_sites.csv'
...
def create_site(site):
    response = nautobot.dcim.sites.create(**site)
...
def create_device(device):
    response = nautobot.dcim.devices.create(**device)

    if response.status_code == 201:
        print(f"Device '{device['name']}' created successfully.")
        return response.id  # Return device ID for further use
...
def create_interface(device_id, interface):
    ...
    response = nautobot.dcim.interfaces.create(**interface)
...
def assign_ip_to_interface(interface_id, ip):
    ...
    response = nautobot.ipam.ip_addresses.create(**ip_data)
...
def main():
    ...
    with open(NETBOX_SITES_PATH, mode='r') as csvfile:
    ...
    with open(NETBOX_DEVICES_PATH, mode='r') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            device = {
                "name": row['Name'],
